...
To create a suitable geographic lookup, a PostgreSQL view was created with one record per ZIP code prefix. Average latitude and longitude were calculated for each ZIP prefix, while the most frequently occurring city and state were retained using PostgreSQL's `MODE()` function.
...
The `order_reviews` table contained multiple review submissions for some orders. Among orders represented in the review data, 547 had more than one review submission.

Among these orders, 202 (37%) had different review scores across submissions, while 345 (63%) retained the same score.
...
- **Investigate the decline in on-time delivery:** Although average delivery time remained stable, the on-time delivery rate declined by 4.9 percentage points. Delivery performance should therefore be monitored against estimated delivery dates, not only average transit time.

- **Prioritize fulfillment improvements for cross-state orders:** Cross-state orders show substantially longer delivery times and delays than same-state orders. Seller coverage and fulfillment routes in high-demand states should be evaluated to identify opportunities for more localized fulfillment.

- **Focus category strategy on the main revenue drivers:** With approximately 80.9% of revenue generated by the top 18 categories, inventory, seller availability, and commercial initiatives can be prioritized around these high-contribution categories while monitoring opportunities in the remaining portfolio.

- **Treat delivery performance as a customer-satisfaction driver:** The strong decline in review scores as delays increase suggests that reducing severe delivery delays could contribute to improved customer satisfaction.
